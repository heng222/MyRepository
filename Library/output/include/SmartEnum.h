...
BEGIN_ENUMEX(Days)
{
    ...
    DECL_ENUM_ELEM_NAME_VALUE_DES(thursday,200,_T("Taday is thrusday.")),
    DECL_ENUM_ELEM_NAME_DES(friday, _T("friday, written in Chinese.")),
...
e.g.
int iWednesday = wednesday;
TString strWednesday = GetNameStringDays(wednesday);
...
*/
    ...
    #define END_ENUMEX( ENUM_NAME ) ENUM_NAME; \
			extern "C++" BSCEG_EXT_API TString GetNameString##ENUM_NAME(enum tag##ENUM_NAME index); \
            extern "C++" BSCEG_EXT_API int GetType##ENUM_NAME(const TString& strName); \
            extern "C++" BSCEG_EXT_API TString GetDes##ENUM_NAME(enum tag##ENUM_NAME index); 
#else

	typedef  struct{int id; LPTSTR name; LPTSTR des;} PACK;
	#define DECL_ENUM_ELEM_NAME( element ) {count++, _T(#element), _T("")}
    #define DECL_ENUM_ELEM_NAME_VALUE( element, value ) {count = ##value, _T(#element), _T("")}
    #define DECL_ENUM_ELEM_NAME_DES( element, des ) {count++ , _T(#element), _T("")}
    #define DECL_ENUM_ELEM_NAME_VALUE_DES( element, value, des ) { count = ##value, _T(#element), des }
    ...
    { \
        ...
        return _T("ERROR - no description"); \
    }
