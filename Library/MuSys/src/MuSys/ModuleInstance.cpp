	...
	m_pLibHandler(NULL)
{
    ...
    if (CSysUtils::IsPsapiSupported())
		m_pLibHandler = new CPsapiHandler(m_pProcesses);
	else
	if (CSysUtils::IsToolHelpSupported())
...
BOOL CPsapiHandler::Initialize()
{
    ...
    if (NULL != m_hModPSAPI)
	{
		...
		m_pfnGetModuleFileNameEx = (PFNGETMODULEFILENAMEEX)
			::GetProcAddress(m_hModPSAPI, "GetModuleFileNameEx");

	}
	bResult = 
		m_pfnEnumProcesses
		&&  m_pfnEnumProcessModules
		&&  m_pfnGetModuleFileNameEx;
...
BOOL CToolhelpHandler::PopulateModules(CModuleInstance* pProcess)
{
	...
	{	
		...
		if (0 != _tcsicmp(pProcess->GetBaseName(), me.szModule))  
		{
...
//
//---------------------------------------------------------------------------
CModuleInstance::CModuleInstance(TCHAR *pszName, HMODULE hModule):
	m_pszName(NULL),
    m_hModule(NULL),
...
TCHAR* CModuleInstance::Get_Name() const
{
	return m_pszName;
}

TCHAR* CModuleInstance::GetBaseName() const
{
	TCHAR *pdest;
	int  ch = '\\';
	// Search backward 
	pdest = _tcsrchr(m_pszName, ch);
	if(pdest != NULL)
		return &pdest[1];
	else
		return m_pszName;
...
void CModuleInstance::Set_Name(TCHAR *pszName)
{
	if (NULL != m_pszName)
		delete [] m_pszName;
    
    size_t iSize = _tcslen(pszName);
	if ((NULL != pszName) && (iSize))
	{
		m_pszName = new TCHAR[iSize + 1];
		_tcscpy_s(m_pszName, iSize, pszName);
	}
	else
	{
