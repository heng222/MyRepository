...
class AFX_EXT_CLASS CXMLFile
{
/// Construction
public:
	CXMLFile();
	CXMLFile(const char* filename, const char* root_name="xmlRoot"); //
...
public:
	...
	*/
	...
	//-------------------------------------------------------------------------------------
	int  GetChildNodeNum(MSXML2::IXMLDOMNodePtr parentNode);
	MSXML2::IXMLDOMNodePtr FindFirstChildNode(MSXML2::IXMLDOMNodePtr parentNode);
	MSXML2::IXMLDOMNodePtr FindNextChildNode(MSXML2::IXMLDOMNodePtr parentNode);
	
	BOOL MyLoad(_bstr_t xml);  //临时交换用
	void MyDetach();//临时交换用
	BOOL GetNodeValue(MSXML2::IXMLDOMNodePtr pCurNode, const char* cstrBaseKeyName, CString &strValue);//临时交换用
	BOOL GetNodeValue(MSXML2::IXMLDOMNodePtr pCurNode, const char* cstrBaseKeyName, int &nValue);//临时交换用

protected:
	MSXML2::IXMLDOMDocument2Ptr MyXmlDocPtr;//临时交换用，相应函数MyLoad，MyDetach
