...
public:
	...
	void operator() (LPCTSTR lpszFormat, ...)
	{
		...
		{
            if (_sntprintf_s(buf1, BUFFER_SIZE, _T("%s(%d):[%X]:%s"), 
		...
		{
			if (_sntprintf_s(buf1, BUFFER_SIZE, _T("%s(%d) : %s"), 
		...
		if (_vsntprintf_s(buf2, BUFFER_SIZE, buf1, va) < 0)
        {
...
#ifdef _DEBUG
#define XTRACE (xtracing_output_debug_string(_T(__FILE__), __LINE__ ))
#define XTRACEERROR (xtracing_output_debug_string(_T(__FILE__), __LINE__ ))
#else
#define XTRACE __noop//((void)0)
#define XTRACEERROR __noop//((void)0)
#endif

#define XTRACERECT(r) TRACE(_T(#r) _T(":  left = %d  top = %d  right = %d  bottom = %d\n"), \
