...
#define EVT_CONSUCCESSED	0x0004	// Zero length message

#define BUFFER_SIZE		4096//MAX_PATH
...
public:
	...
	bool IsOpen() const;	// Is Socket valid and connected?
	bool IsOpenEx() const;	// Is Socket valid?
	...
	bool CreateSocket(LPCTSTR strip, LPCTSTR strServiceName, int nProtocol, int nType, UINT uOptions = 0);
	// Create a socket, connect to (Client side)
	bool ConnectTo(LPCTSTR strDestination, LPCTSTR strServiceName, LPCTSTR strSource, LPCTSTR strSourceServiceName, int nProtocol, int nType);
...
protected:
	...
	static HANDLE	m_hPostMutex;		// Mutex object
	bool		m_bActive;     //for heart test
// SocketComm - function
protected:
