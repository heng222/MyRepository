...
void CInfPromptDlg::OnButtonShowNewMsg() 
{
	...
	COleDateTime tSpeTime = m_newMessage.GetAt(iCurNo)->m_tSpeTime;

	//删除查看了的消息
	m_newMessage.DeleteItem( iCurNo );
	ShowNewMsg();
	
	if ( strMsg.CompareNoCase("施工登记请求") == 0 )
	{
		CConstructRequest dlg;
		dlg.m_byConKind = CONSTREGISTER;
		dlg.m_byOrder = iOrder;
		
		dlg.DoModal();
	}
	else if ( strMsg.CompareNoCase("施工消记请求") == 0 )
	{
		CConstructRequest dlg;
		dlg.m_byOrder = iOrder;
		dlg.m_byConKind = CONSTCLEARREG;
		
		dlg.DoModal();
	}
	else if ( strMsg.CompareNoCase("施工封锁命令") == 0 )
	{
		m_pMainFrame->m_pDdmlxdDlg->SetChecked(TRUE);
		m_pMainFrame->m_pDdmlxdDlg->SetComKind(DISPHKINDCLOSE);
		m_pMainFrame->m_pDdmlxdDlg->SetOrder(iOrder);
		m_pMainFrame->m_pDdmlxdDlg->SetRCDateTime(tSpeTime);
	...
	{
		m_pMainFrame->m_pDdmlxdDlg->SetChecked(TRUE);
		m_pMainFrame->m_pDdmlxdDlg->SetComKind(DISPHKINDOPEN);
		m_pMainFrame->m_pDdmlxdDlg->SetOrder(iOrder);
		m_pMainFrame->m_pDdmlxdDlg->SetRCDateTime(tSpeTime);
