...
CMainFrame::CMainFrame()
{
    ...
    //GetActiveView()->IsKindOf(RUNTIME_CLASS(CMainUIView));
}
...
void CMainFrame::ActiveEvents()
{
    ...
    // 数据库检测
    IMsgDBCntTest msg1;
    msg1.SetType(IMsgDBCntTest::E_REQUEST);
    NbsSendModuleMsg(&msg1);

    // 创建气象信息视图
    CView* pView = theApp.GetView(RUNTIME_CLASS(CWeatherView));
    TImcMsg msg(EMSG_CREATE_RGNVIEW,ID_MODULE_USERIF,ID_MODULE_DATABASE);
    msg.m_wParam = WPARAM(pView);
...
LRESULT CMainFrame::OnRecvUserMsg(WPARAM wParam,LPARAM lParam)
{
    ...
    if (wMsgType==ESYS_NEW_LOG)
    {
        ...
        CMainUIView* pUIView = (CMainUIView*)(theApp.GetView(RUNTIME_CLASS(CMainUIView)));
        if ( pUIView->GetSafeHwnd()!=NULL )
        {
            pUIView->AddLiveLog(pNewLog);
        }
    }
