...
CTrayIcon::CTrayIcon(UINT uID)
{
	...
	AfxLoadString(uID, m_nid.szTip, sizeof(m_nid.szTip)/sizeof(m_nid.szTip[0]));
}
...
BOOL CTrayIcon::SetIcon(HICON hicon, LPCTSTR lpTip) 
{
	...
	if (lpTip)
    {
        int iCount = sizeof(m_nid.szTip)/sizeof(m_nid.szTip[0]);
		_tcsncpy_s(m_nid.szTip,iCount ,lpTip, _tcslen(lpTip));
    }
...
LRESULT CTrayIcon::OnTrayNotification(WPARAM wID, LPARAM lEvent)
{
	...
	{
		// double click: Show the notify window
		//::SendMessage(m_nid.hWnd, WM_COMMAND, pSubMenu->GetMenuItemID(0), 0);
        ::ShowWindow(m_nid.hWnd,SW_RESTORE);
        //Shell_NotifyIcon(NIM_DELETE, &m_nid); //É¾³ýÏµÍ³ÍÐÅÌÍ¼±ê  
	}
	return 1; // handled
}
