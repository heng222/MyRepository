...
CSortListCtrl::CSortListCtrl(TCreateStyle iCreateStyle)
	: m_iNumColumns( 0 )
	, m_iSortColumn( -1 )
...
void CSortListCtrl::PreSubclassWindow()
{
    ...
    if (m_iCreateStyle==ECS_ADDRESOURCE)
    {
...
BOOL CSortListCtrl::SetHeadings( const CString& strHeadings )
{
	...
	for( ;; )
	{
		...
		const int iWidth = _tcstol(strHeadings.Mid( iStart, iSemiColon - iStart ),NULL,10); 
...
int CSortListCtrl::AddItemAndImage(int nImage,LPCTSTR pszText, ... )
{
...
int NumberCompare( LPCTSTR pszNumber1, LPCTSTR pszNumber2 )
{
	...
	const int iNumber1 = _tcstol(pszNumber1,NULL,10);
	const int iNumber2 = _tcstol(pszNumber2,NULL,10); 
...
{
	const int iYear1 = _tcstol(strDate1.Mid( 6, 4 ),NULL,10); 
	const int iYear2 = _tcstol(strDate2.Mid( 6, 4 ),NULL,10); 
	...
	const int iMonth1 = _tcstol(strDate1.Mid( 3, 2 ),NULL,10); 
	const int iMonth2 = _tcstol(strDate2.Mid( 3, 2 ),NULL,10); 
	...
	const int iDay1 = _tcstol(strDate1.Mid( 0, 2 ),NULL,10); 
	const int iDay2 = _tcstol(strDate2.Mid( 0, 2 ),NULL,10); 
...
int CSortListCtrl::InsertItemA(int iIndex, LPCTSTR pszText, ...)
{
	iIndex = InsertItem( iIndex, pszText );
	LPTSTR* arrpsz = new LPTSTR[ m_iNumColumns ];
	COLORREF * clrText = new COLORREF[ m_iNumColumns ];
	COLORREF * clrBak= new COLORREF[ m_iNumColumns ];
