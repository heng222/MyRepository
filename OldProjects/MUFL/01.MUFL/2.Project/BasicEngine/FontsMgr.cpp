...
BOOL  IsExist(vector<CFontInfo>& fontList, const TString& strFontName);
...
*/
void EnumFontInfo(vector<CFontInfo>& fontInfor,const TString& strFontName)
{
    HDC hdc = ::GetDC(NULL);
    ...
    {
        _tcscpy_s(stLogFont.lfFaceName,strFontName.c_str());
    }
							...
							LPARAM lParam)             // application-defined data
{
    ...
    //TString strFontName = bTrueType ? pEnumLogFont->elfFullName : lpelfe->lfFaceName;
...
//////////////////////////////////////////////////////////////////////////
// 
void EnumFontHeight(const TString& strFontName,list<int>& heightList )
{
							   ...
							   LPARAM lParam)             // application-defined data
{
	...
	/*TString elfScript1(_T("Î÷·½"));
	TString elfScript2(_T("Western"));
...
// in point
BYTE GetFontMinHeight(const TString& strFontName)
{
    list<int> fontSize;
...
// 
int  PointToLogicUnit(int iPointValue,const TString& strFontName)
{
    if (iPointValue<=0)
    {
        return 0;
