...
void CDaoC::DrawGeneralDC(CDC *pDC,BOOL bInfInvalid)
{
	...
	else
	{
		...
		else if(m_iRWCBand&WHITE_BAND){
			clrDraw=SUOBI_CLR;
		}
		else if(m_iRWCBand&RED_BAND){
...
void CDaoC::DrawTGQ(CDC *pDC,BOOL bInfInvalid)
{
	...
	//再用指定的颜色画新区域
	iClr = bInfInvalid ? CLR_INVALIDATE : KONGXIAN_CLR;
...
void CDaoC::DrawSlf(CDC* pDC, int mDvcSta)
{
	if ( m_iType == DT_TUOGUIQI) {
		DrawTGQ(pDC,mDvcSta==DS_INVALID);
	}
