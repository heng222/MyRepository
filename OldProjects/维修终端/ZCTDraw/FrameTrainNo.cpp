...
#ifdef _DEBUG
	#define  TRUCKTRAINNONUM	2			//车次号大于TRUCKTRAINNONUM时为货车
#else
	#define  TRUCKTRAINNONUM	5			//车次号大于TRUCKTRAINNONUM时为货车
#endif
...
void CFrameTrainNo::GetRectangle(CRect *pRtTime, CRect *pRtText, CRgn *pRgBack)
{
	...
	CPoint point[5];
	float fShrink = (m_iTimeExcur == 0) ? (float)(9/10.0) : (float)(3/4.0);
	////
