...
//CReadStationInf* CReadStationInf::_pInstance = NULL;
...
CReadStationInf::CReadStationInf()
{
	...
	m_strCurDirectory = CString(cTemp) + "\\";
}
...
void CReadStationInf::ReleaseInstance()
{
	...
	CString strExePath;
	GetCurrentDirectory(MAX_PATH,cTemp);
	strExePath.Format("%s\\ini\\StationInf.ini",cTemp);
	CString strStaID;
...
BOOL CReadStationInf::ReadFile(LPCSTR lpFileName/*=""*/)
{
	...
	//施工种类的个数
	CString strAppName;
	StationNum = GetPrivateProfileInt("SYSTEM","StationCount",-1,lpFileName);
...
CString CReadStationInf::GetName(const WORD &wStationNo,BOOL bFlag/*=TRUE*/) const
{
...
CString CReadStationInf::GetDirectory(const WORD& wStationNo,BOOL bAbsolute) const
{
	...
	while( p!=NULL )
	{
		...
		{
			if (bAbsolute) {
				return  (m_strCurDirectory + p->strDirectory + "\\");
			}
			else
			{
...
CString CReadStationInf::GetDirectory(const CString& strStationName) const
{
	StationNode *p = m_pHead;
	while( p!=NULL )
	{
		if(p->strName == strStationName )
		{
			return p->strDirectory;
		}
		p = p->Next;
