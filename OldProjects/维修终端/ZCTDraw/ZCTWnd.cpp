...
//------------------------------------------------------------------------------
/////////////////////////////////////////////////////////////////////////////
//站场图显示所用数据的相对路径(当前目录在StationInf.ini中定义)

#define DATAPATH_OFFSET		"Graphics\\offset.dat"		//
#define DATAPATH_SYARD		"Graphics\\yard.dat"		//
#define DATAPATH_MYARD		"Graphics\\yard_M.dat"		//
#define DATAPATH_MASK		"Graphics\\mask.ini"		//
#define DATAPATH_SFRAME		"Graphics\\frame.dat"		//
#define DATAPATH_MFRAME		"Graphics\\frame_M.dat"		//
#define DATAPATH_GUDAO		"Graphics\\gd.dat"			//
#define DATAPATH_DESK		"Graphics\\desk.dat"		//	
#define DATAPATH_JINLU		"jl.ini"					//	


// CZCTWnd
IMPLEMENT_DYNCREATE(CZCTWnd, CWnd)

CZCTWnd::CZCTWnd():m_hPMHandle(NULL),m_pTempDevice(NULL),
	m_dwShowStyle(0xFFFFFFFF),	m_wOperatorAttr(OPER_FUNCTNBTN),
	m_nMapMode(MM_ANISOTROPIC),	m_bSingle(TRUE),m_sizeShrink(1,1),
	m_iStationNo(0),m_sizeExcursion(0,0),m_iHandFlag(MSFL_INVALID),
...
BOOL CZCTWnd::ReadDesk(CString strFileName)
{
	...
	for(int i=0;i<iFuncBtnNums;i++)
	{
	    ...
	    sscanf(strLine,"%d %d %x %d %x %d %d %d %s",
			...
			/*&pBtn->m_iClr1*/&iTempData ,/*&pBtn->m_iFlash*/&iTempData,pBtn->m_strName.GetBuffer(20)
			);
...
BOOL CZCTWnd::ReadGd(CString strFileName)
{
	...
	for(i=0;i<iQjStatusNums;i++)
	{
	    ...
	    sscanf(strLine,"%d %d %d %d %d %d %d %d %10s",
			...
			&pBtn->m_iStationNo2 ,&pBtn->m_iXHNo2,&pBtn->m_iCommandID2,pBtn->m_strName .GetBuffer(10));
		pBtn->m_strName .ReleaseBuffer();
...
BOOL CZCTWnd::ReadYard(CString strFileName)
{
	...
	for( i=0;i<iDCNums;i++)
	{
		...
		sscanf(strLine,"%d %d %d %d %d %d %d %d %d %d %d %d %x %s",
			...
			&pDaoC->m_iType,pDaoC->m_strName.GetBuffer(20)	);
	...
	for(i=0;i<iXHNums;i++)
	{
		...
		sscanf(strLine,
			...
			&pXHJ->m_iTx ,&pXHJ->m_iTy ,pXHJ->m_strName.GetBuffer(20));
...
//*/
	...
	for(i=0;i<iWCQDNums;i++)
	{
		...
		sscanf(	strLine,
		    ...
		    &pWcQD->m_iCommandID,pWcQD->m_strName.GetBuffer(20)
			);
	...
	for(i=0;i<iBtnNums;i++)	{
		...
		sscanf(strLine,"%d %d %x %d %u %u %d %d %d %d %s",
			...
			&pAN->m_deltaY,&pAN->m_bShow,&pAN->m_bHidAble,pAN->m_strName.GetBuffer(20)
			);
...
//*/
	...
	for(i=0;i<iBSDNums;i++)	{
		...
		sscanf(strLine,"%d %d %d %d %d %d %d %d %20s",&pBSD->m_iType,
			...
			&pBSD->m_T_pt.x,&pBSD->m_T_pt.y,&pBSD->m_bHidAble,pBSD->m_strName.GetBuffer(20));
...
BOOL CZCTWnd::Create(CWnd *pReferWnd,CWnd*pCanvasWnd,HWND hMsgHandle)
{
	...
	if (m_bSingle){
		if (!CWnd::Create(NULL,"",WS_CHILD,
				CRect(0,0,MAP_WIDTH,MAP_HEIGHT),pReferWnd,m_iStationNo+5000)) {
			return FALSE;
		} 
		//
		m_pCanvasWnd = this;
...
/*		//若正在区故解
		if ( m_pFuncBtnActive){	
			if ( m_pFuncBtnActive->m_iCommandID == CFuncBtn::QGJ){
				pWcQD->DrawQGJ(pDC,TRUE);
			}	
		}
		else{
			//显示无岔区段名称
			pWcQD->ShowName (pDC,m_dwShowStyle & SHOW_WCQDNAME);
		}
*/
		BOOL bFlag = FALSE;
		...
		//pWcQD->ShowName (pDC,bFlag,m_dwShowStyle&SHOW_WCQDNAME);
...
void CZCTWnd::PrintZCT(CPoint ptOrigin,CSize SizeZoom)
{
	...
	CSize size(MAP_WIDTH,MAP_HEIGHT);
...
CAnN* CZCTWnd::IsInAnN(CDC* pDC,CPoint pt)
{
	...
	while( pos ){
		...
		//转换成设备坐标
		CRect rectDevice = CRect(pAN->m_pt.x,	pAN->m_pt.y,
							pAN->m_pt.x+AN_WIDTH,	pAN->m_pt.y+AN_HEIGHT);
...
CFuncBtn* CZCTWnd::IsInFuncBtn(CDC* pDC,CPoint pt)
{
	POSITION pos = m_FuncBtnList.GetHeadPosition();	
	while(pos!=NULL){
		CFuncBtn*	pFuncBtn = m_FuncBtnList .GetNext(pos);
		
		CRect rectDevice=CRect(pFuncBtn->m_pt.x-FUNBTNRADIUS,pFuncBtn->m_pt.y-FUNBTNRADIUS,
			pFuncBtn->m_pt.x+FUNBTNRADIUS,pFuncBtn->m_pt.y+FUNBTNRADIUS);
...
CGdStatus* CZCTWnd::IsInGdFsLight(CDC* pDC,CPoint pt)
{
	POSITION pos = m_GdStatusList.GetHeadPosition();
	while(pos!=NULL){
		CGdStatus* pGdLight = m_GdStatusList.GetNext(pos);
	
		CRect rectDevice=CRect(pGdLight->m_pt.x,pGdLight->m_pt.y-GDLIGHTRADIUS,
			pGdLight->m_pt.x+GDLIGHTRADIUS*2,pGdLight->m_pt.y+GDLIGHTRADIUS);
...
CQjStatus* CZCTWnd::IsInQjLigth(CDC* pDC,CPoint pt)
{
	POSITION pos = m_QjStatusList.GetHeadPosition();
	while( pos!=NULL )
	{
		CQjStatus*	pQJLight = m_QjStatusList.GetNext(pos);
		
		CRect rectDevice=CRect(pQJLight->m_pt.x,pQJLight->m_pt.y-QJLIGHTRADIUS,
			pQJLight->m_pt.x+QJLIGHTRADIUS*2,pQJLight->m_pt.y+QJLIGHTRADIUS);
...
CWcQD*	CZCTWnd::IsInWcQDName(CDC* pDC,CPoint pt)
{
	POSITION pos = m_WcQDList.GetHeadPosition();
	while (pos)
	{
		CWcQD* pWCQD = m_WcQDList.GetNext(pos);
		CRect rectDevice=CRect(pWCQD->m_pt.x,pWCQD->m_pt.y,pWCQD->m_pt.x+AN_WIDTH*4,pWCQD->m_pt.y+AN_HEIGHT);
		pDC->LPtoDP(rectDevice);
		if(rectDevice.PtInRect(pt) && pWCQD->m_bCanQGJ)
		{
			return pWCQD;
		}
	}
...
CDCAN*	CZCTWnd::IsInDChaName(CDC* pDC,CPoint pt)
{
	POSITION pos = m_DCANList.GetHeadPosition();
	...
	{
		CDCAN* pDCAN = m_DCANList.GetNext(pos);
		CRect rectDevice=CRect(pDCAN->m_pt.x,pDCAN->m_pt.y,pDCAN->m_pt.x+AN_WIDTH,pDCAN->m_pt.y+AN_HEIGHT);
...
#ifdef _DEBUG
		//m_fStationShrink = 1.9;
#endif
		if (m_nMapMode == MM_ISOTROPIC)
        {
			pDC->SetWindowExt(int(MAP_WIDTH/m_fStationShrink),
				int(MAP_HEIGHT/m_fStationShrink));	//logic coordinates
		}
		else
        {
			pDC->SetWindowExt(int(MAP_WIDTH),int(MAP_HEIGHT));	//logic coordinates
		}
...
void CZCTWnd::OnTimer(UINT nIDEvent) 
{
	...
	{
		if ( m_pFuncBtnActive )	{//处于激活状态的功能按钮闪烁
...
/*
		if ( m_dwShowStyle & SHOW_BSHIDENG)
		{
			...
			while(pos!=NULL){
				CBSD *pBSD = m_BSDList.GetNext(pos);
				if(pBSD->m_iStatus > 0x11){
					pBSD->m_iPrevStatus = pBSD->m_iStatus;
					if( m_bFlashFlag )	{
						switch(pBSD->m_iStatus) {
						case COL_YF:	pBSD->m_iStatus=COL_YELLOW;		break;
						case COL_RF:	pBSD->m_iStatus=COL_RED;		break;
						case COL_GF:	pBSD->m_iStatus=COL_GREEN;		break;
						case COL_WF:	pBSD->m_iStatus=COL_WITHE;		break;
						}
					}
					else if(pBSD->m_iStatus > 0x11){
							pBSD->m_iStatus=COL_BLACK;
					}
					
					pBSD->FreshBSD(&dc,FALSE);
					pBSD->m_iStatus=pBSD->m_iPrevStatus;
				}
			}
		}
*/		
		...
		while ( pos ){
			...
			if ( pDaoC->IsJC())	{
				if ( m_bFlashFlag ){
					pDaoC->DrawDcc(pDC,/*pDaoC->m_pt.x,pDaoC->m_pt.y,*/
						pDaoC->m_deltaB.x,pDaoC->m_deltaB.y,REDCOLOR,REDCOLOR,FALSE,TRUE);
					//
					pDaoC->DrawDcc(pDC,/*pDaoC->m_pt.x,pDaoC->m_pt.y,*/
						pDaoC->m_deltaC.x,pDaoC->m_deltaC.y,REDCOLOR,REDCOLOR,FALSE,TRUE);
				}
				else{
					pDaoC->DrawDcc(pDC,/*pDaoC->m_pt.x,pDaoC->m_pt.y,*/
						pDaoC->m_deltaB.x,pDaoC->m_deltaB.y,BLACKCOLOR,BLACKCOLOR,FALSE,TRUE);
					//
					pDaoC->DrawDcc(pDC,/*pDaoC->m_pt.x,pDaoC->m_pt.y,*/
						pDaoC->m_deltaC.x,pDaoC->m_deltaC.y,BLACKCOLOR,BLACKCOLOR,FALSE,TRUE);
				}
			}
		}
		//信号机闪烁
...
void CZCTWnd::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	{
		//m_pTempFuncBtn = IsInFuncBtn(pDC,point) ;
		m_pTempDevice = IsInFuncBtn(pDC,point) ;
		//if ( m_pTempFuncBtn) {
		if ( m_pTempDevice) {
			m_iHandFlag = MSFL_FUNTBTN;
			return;
		}
	}
	
	// 检测当前鼠标位置是否在调车或列车或道岔或引导按钮里
	m_pTempDevice = IsInAnN(pDC,point);
	if ( m_pTempDevice )
	{
		//根据条件检测当前鼠标位置是否在调车按钮里
		//没有功能按钮激活或激活的功能按钮为总人解或总取消或重开信号时，bTmpFlag为真(可操作调列车按钮)
		if ( m_pFuncBtnActive == NULL)	{
			bTmpFlag = TRUE;
		}
		else{
			bTmpFlag  = m_pFuncBtnActive->m_iCommandID == CFuncBtn::ZRJ; 
			bTmpFlag |= m_pFuncBtnActive->m_iCommandID == CFuncBtn::ZQX;
			bTmpFlag |= m_pFuncBtnActive->m_iCommandID == CFuncBtn::CKXH;
		}
		...
		if ( m_pFuncBtnActive != NULL )	{
			bTmpFlag  = m_pFuncBtnActive->m_iCommandID == CFuncBtn::ZDW;
			bTmpFlag |= m_pFuncBtnActive->m_iCommandID == CFuncBtn::ZFW;
			bTmpFlag |= m_pFuncBtnActive->m_iCommandID == CFuncBtn::DS;
			bTmpFlag |= m_pFuncBtnActive->m_iCommandID == CFuncBtn::DJ;
			bTmpFlag |= m_pFuncBtnActive->m_iCommandID == CFuncBtn::DF;
			bTmpFlag |= m_pFuncBtnActive->m_iCommandID == CFuncBtn::JF;
			if ( bTmpFlag && (m_pTempDevice->GetType()==CAnN::BTN_TYPE_CA) && (m_wOperatorAttr & OPER_DAOCHABTN) )
			{
				m_iHandFlag = MSFL_DAOCHAN;
				return;				
			}
		}
	}
	...
	{
		//m_pTempGdLight = IsInGdFsLight(pDC,point);
		m_pTempDevice = IsInGdFsLight(pDC,point);
		
		//if ( m_pTempGdLight ){
		if ( m_pTempDevice ){
			m_iHandFlag = MSFL_GDLIGHT;
			return;
		}
	}
	...
	{
		//m_pTempQjLight = IsInQjLigth(pDC,point);
		m_pTempDevice = IsInQjLigth(pDC,point);
		
		//if(m_pTempQjLight)	{
		if(m_pTempDevice){			
			m_iHandFlag = MSFL_QJLIGHT;
			return;
		}
	}
	...
	{
		//检测当前鼠标位置是否在无岔区段名称里
		//m_pTempWcQuDuan = IsInWcQDName(pDC,point);
		m_pTempDevice = IsInWcQDName(pDC,point);
		//if ( m_pTempWcQuDuan )	{
		if ( m_pTempDevice )	{	
			m_iHandFlag = MSFL_WCQDNAME;
			return;
		}

		//检测当前鼠标位置是否道岔名称里
		//m_pTempDCQuDuan = IsInDChaName(pDC,point);
		m_pTempDevice = IsInDChaName(pDC,point);
		//if( m_pTempDCQuDuan )	{	
		if( m_pTempDevice )	{	
			m_iHandFlag = MSFL_DCQDNAME;
			return;
		}
	}

	m_iHandFlag = MSFL_INVALID;
	// TODO: Add your message handler code here and/or call default
	CWnd::OnMouseMove(nFlags, point);
}
...
void CZCTWnd::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	//鼠标位置在桌面功能按钮里
	if ( m_iHandFlag == MSFL_FUNTBTN)	{
		//m_pTempFuncBtn->DrawSlf(pDC,DS_DOWN);
		m_pTempDevice->DrawSlf(pDC,DS_DOWN);
	}
	//在调列车按钮里或道岔按钮里
	else if ( m_iHandFlag == MSFL_DLCHEAN || m_iHandFlag == MSFL_DAOCHAN)	{
		m_pTempDevice->DrawSlf(pDC,DS_DOWN);
	}
...
void CZCTWnd::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if ( m_iHandFlag == MSFL_FUNTBTN )	
	{
		...
		//得到当前(鼠标点击的)功能按钮的ID
		//int	iFuncBtnCmdID = m_pTempFuncBtn->m_iCommandID;
		...
		if ( iFuncBtnCmdID == CFuncBtn::XYZS || iFuncBtnCmdID == CFuncBtn::SYZS)
		{
			...
			if (dlg.DoModal() == IDOK) {
				...
				if (dlg.DoModal() == IDOK) {
					//if ( m_pTempFuncBtn->m_iStatus == DS_DOWN ){
		...
		*/
		else if(iFuncBtnCmdID == CFuncBtn::QGJ )
		{
			...
			if ( bFlag )
			{
				...
				//重新设置激活的按钮为当前按下的按钮
				//m_pFuncBtnActive = m_pTempFuncBtn;
		...
		//保存上次操作的功能按钮
		//m_pFuncBtnLast = m_pTempFuncBtn;
		m_pFuncBtnLast = (CFuncBtn*)m_pTempDevice;

		//重画功能按钮
		//m_pTempFuncBtn->DrawSlf(pDC,DS_COMM);
	...
	/*------------  当前鼠标位置在股道封锁灯里  -------------------------*/
	else if( m_iHandFlag == MSFL_GDLIGHT )
	{
		sStaMsg.iValueRelative = CFuncBtn::GDFS;
		//::SendMessage(m_hMsgHandle,UM_FORMSTATION,(WPARAM)&sStaMsg,(LPARAM)m_pTempGdLight);
...
//*/
	...
	/* ---------- 当前鼠标位置在区间封锁灯里 ----------------------------*/
	else if( m_iHandFlag == MSFL_QJLIGHT )
	{
		sStaMsg.iValueRelative = CFuncBtn::QJFS;
		//::SendMessage(m_hMsgHandle,UM_FORMSTATION,(WPARAM)&sStaMsg,(LPARAM)m_pTempQjLight);
		::SendMessage(m_hMsgHandle,UM_FORMSTATION,(WPARAM)&sStaMsg,(LPARAM)m_pTempDevice);
	...
	/*---------------当前鼠标位置在调车或列车按钮里----------------------*/
	else if ( m_iHandFlag == MSFL_DLCHEAN )
	{
		BOOL bConfirm;
	...
	/*-------------------当前鼠标位置在道岔按钮里-------------------------*/
	else if ( m_iHandFlag == MSFL_DAOCHAN )	{
		//向父窗口发消息
		sStaMsg.iValueRelative = m_pFuncBtnActive->m_iCommandID;
		::SendMessage(m_hMsgHandle,UM_FORMSTATION,(WPARAM)&sStaMsg,(LPARAM)m_pTempDevice);
	...
	/*-----------------位置在无岔区段名称里------------*/
	/*-----------------位置在道岔名称里----------------*/
	else if (m_iHandFlag == MSFL_WCQDNAME || m_iHandFlag == MSFL_DCQDNAME)
	{
		sStaMsg.iValueRelative = CFuncBtn::QGJ;
		/*
		if ( m_iHandFlag == MSFL_WCQDNAME )		{
			//向父窗口发消息
			::SendMessage(m_hMsgHandle,UM_FORMSTATION,(WPARAM)&sStaMsg,(LPARAM)m_pTempWcQuDuan->m_iCommandID);
		}
		else	{
			::SendMessage(m_hMsgHandle,UM_FORMSTATION,(WPARAM)&sStaMsg,(LPARAM)m_pTempDCQuDuan->m_iCommandID);
...
void CZCTWnd::OnPaint()
{
	...
	pDC->Rectangle (CRect(0,0,MAP_WIDTH,MAP_HEIGHT));
...
{
	//光标置“手”状或光标位置在车次框里时，不弹出菜单
	if (pWnd != this || m_iHandFlag != MSFL_INVALID){//|| m_StationMenu.GetMenuItemCount()>11) {
		return;
	}
	...
	int iOrginCount = m_StationMenu.GetMenuItemCount();
	if (m_dwShowStyle&SHOW_POPMENU) {
		...
		m_StationMenu.AppendMenu(nFlags,(UINT)(HMENU)popSubMenu,"自由缩放(&S)\t");
		...
		//m_StationMenu.AppendMenu(nFlags,ID_SK_ZOOMIN,"放大站场图＋\t");
		//m_StationMenu.AppendMenu(nFlags,ID_SK_ZOOMOUT,"缩小站场图－\t");
	}
	m_StationMenu.TrackPopupMenu(TPM_LEFTALIGN|TPM_LEFTBUTTON,point.x,point.y,this);

	//
	UINT uPresentCount = m_StationMenu.GetMenuItemCount();
	for(UINT i=0; i<uPresentCount-iOrginCount; i++){
		m_StationMenu.DeleteMenu(iOrginCount,MF_BYPOSITION);
	}
}
...
BOOL CZCTWnd::ReadStationInf(CString strPath, int iReadMode)
{
	...
	if (iReadMode & RDMD_OFFSET) {
		if (!ReadOffset(strPath+DATAPATH_OFFSET)) {
	...
	if (iReadMode & RDMD_SYARD) {
		if (!ReadYard(strPath+DATAPATH_SYARD)) {
			TRACE("Read single yard.dat failure!\n");
			return FALSE;
		}
	}
	//
	if (iReadMode & RDMD_MYARD) {
		if (!ReadYard(strPath+DATAPATH_MYARD)) {
	...
	if (iReadMode & RDMD_MASK) {
		if (!m_ParseStaInfo.ReadMask(strPath+DATAPATH_MASK)) {
	...
	if (iReadMode & RDMD_SFRAM) {
		if ( !m_FrameTrainNoList.ReadFrameInf(strPath+DATAPATH_SFRAME,
	...
	if (iReadMode & RDMD_MFRAM) {
		if (!m_FrameTrainNoList.ReadFrameInf(strPath+DATAPATH_MFRAME,
				m_sizeShrink,m_sizeExcursion)) {
	...
	if (iReadMode & RDMD_GUDAO) {
		if (!ReadGd(strPath+DATAPATH_GUDAO)) {
	...
	if (iReadMode & RDMD_DESK) {
		if (!ReadDesk(strPath+DATAPATH_DESK)) {
	...
	if (iReadMode & RDMD_JINLU) {
		if ( !m_InterlockJuge.m_EntRailWay.ReadERFile(strPath+DATAPATH_JINLU) ) {
			TRACE("Read jl.ini failure!\n");
			return FALSE;
		}
		if ( !m_InterlockJuge.m_PasRailWay.ReadFile(strPath+DATAPATH_JINLU)) {
