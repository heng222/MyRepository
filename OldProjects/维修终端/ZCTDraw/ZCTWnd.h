...
public:
	 ...
	 */
	enum MapSize{
		MAP_WIDTH  = 1280/*站场图的逻辑宽度*/,
		MAP_HEIGHT = 1024/*站场图的逻辑高度*/
	};
...
private:
	 ...
	 *	临时设备指针。
		...
		m_iHandFlag == MSFL_GDLIGHT时，保存股道指针
		m_iHandFlag == MSFL_QJLIGHT时,保存区间指针
	 */
...
private:
	 ...
	 *	
	 */
	CGdStatus*	IsInGdFsLight(CDC* pDC,CPoint pt);
	CFuncBtn*	IsInFuncBtn (CDC* pDC,CPoint pt);
	CAnN*		IsInAnN (CDC* pDC,CPoint pt);
	CQjStatus*	IsInQjLigth(CDC* pDC,CPoint pt);
	CWcQD*		IsInWcQDName(CDC* pDC,CPoint pt);
	CDCAN*		IsInDChaName(CDC* pDC,CPoint pt);
