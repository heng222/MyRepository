...
BOOL CCityMgr::ReadInf()
{
    ...
    while (fileCitys.ReadString(strLine))
    {
        ...
        {
            pField->m_strCityName = GetSFStr((LPCSTR)strLine,1,_T("=")).c_str();
        }
        else if (strLine.Find(_T("Item"))!=-1)
        {
            strLine.TrimLeft("Item=");
            int iAttri = (int)GetNFStr((LPCSTR)strLine,0,_T(","));
            CString strID = GetSFStr((LPCSTR)strLine,1,_T(",")).c_str();
            CString strName = GetSFStr((LPCSTR)strLine,2,_T(",")).c_str();
            WORD wCityIndex = (WORD)GetNFStr((LPCSTR)strLine,3,_T(","));
            ...
            while (pos)
            {
                ...
                {
                    strNewLine = strNewLine + ","+GetSFStr(str,1,_T(","));
                    break;
                }
            }
