...
BOOL CUIModule::OnInitModule()
{
    ...
    if ( IsOneInstance() )
    {
        ::MessageBox(NULL,_T("程序的一个实例已处于运行状态!"),"警告",MB_ICONEXCLAMATION);
        return FALSE;
    }
...
void CUIModule::OnRecvIMCMessage(PTImcMsg aImcMsg)
{
    ...
    if (ESYS_NEW_LOG == wMsgType)
    {
        ...
        // Show the message
        if (m_dlgStart.GetSafeHwnd() && m_dlgStart.IsWindowVisible())
        {
            m_dlgStart.SetFocus();
    ...
    else if (EMSG_SHOW_TIPWND==wMsgType)
    {
        m_dlgStart.ShowWindow(SW_SHOW);
    }
    else if (EMSG_CLOSE_TIPWND==wMsgType)
    {
...
{
    m_hOneInstance=CreateMutex(NULL,FALSE,_T("NaviServer服务器-交通气象发布"));
