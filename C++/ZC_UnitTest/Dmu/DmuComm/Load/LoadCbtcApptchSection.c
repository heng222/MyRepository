...
{
    BnfCbtcApptchSectionStruct* BnfCbtcApptchSectionStru = NULL;/*CbtcApptchSection信息结构体指针*/
    ...
    if((0 == retVal)&&(tempDataLen > 0u))
    {
        ...
        if(NULL != pTempData)
        {
            ...
            if((0 == retVal)&&(0u == (tempDataLen % CBTCAPPTCHSECTION_LINE_BYTE_NUM)))
            {
                ...
                {
                    BnfCbtcApptchSectionStru[i].cbtcApptchSectionId = ShortFromChar(pTempData);                        /*索引编号*/
                    pTempData += 2;
