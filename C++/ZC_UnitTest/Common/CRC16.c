...
*  创建时间 ：   2008.07.29
*  作者     ：   
*  功能描述 ：   用并行的方法快速的计算一组数据的16位CRC校验     
*                POLY：0x1021----使用的生成多项式                                                                       
*  修改记录 ：  
*
********************************************************/ 
...
*  功能描述： CRC16函数，注意：计算CRC是从输入数据第一个字节的最高BIT开始的
            采用的多项式0x1021，初始值为0x0000,未镜像，结果异或系数为0x0000.
            如果DataLen=0，返回值也为0.
*  参数说明： pData,    数据指针
*             DataLen,  数据长度（字节）
*  返回值：   返回值为CRC值.
...
UINT16_S Crc16(const UINT8_S *pData,UINT32_S DataLen)
{
    ...
    const static UINT16_S CrcTable16[256] = {
    	...
    	if(NULL != pData)
        {
            while( i < DataLen ) 
            {   
                /*中Crc左移8位（Crc<<8）实际中是Crc向前挪动一个字节，最后一字节清0。
                Crc右移8位（Crc>>8）实际中只需取Crc这个变量的最高字节就可以了。*/  
                Crc = (UINT16_S)(((UINT32_S)Crc<<8) ^ CrcTable16[ ((UINT32_S)Crc>>8) ^ pData[i]  ]) ;   
                i++;
            }
