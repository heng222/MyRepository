...
ON_REGISTERED_MESSAGE(UWM_CHAR, OnMyWMChar)
...
LRESULT CHookDlg::OnMyMouseMove(WPARAM, LPARAM)
{
    CPoint pt;
    GetCursorPos(&pt);
    CString s;
...
// 
LRESULT CHookDlg::OnMyWMChar(WPARAM wParam, LPARAM lParam)
{
    if (!::isprint(wParam))
    {
        return 1L;
    ...
    CString strOldText;
    m_ctrlEdit.GetWindowText(strOldText);
    CString cChar = (TCHAR) wParam;
    m_ctrlEdit.SetWindowText(strOldText + cChar);
